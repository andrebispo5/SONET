    ...
    public void contextInitialized(ServletContextEvent servletContextEvent) {
        try {
            ...
            for (Organization o : orgs) {
			 ...
			 if(removes.size() != 0){
				 BulkResponse br2 = businessLifeCycleManager.deleteOrganizations(removes);
				 System.out.println("Applied deleteOrganizations");
				 if (br2.getStatus() == JAXRResponse.STATUS_SUCCESS)
		                System.out.println("Deleted completed successfully.");
		         else
		                System.out.println("Fail Deleting Organizations.");
			}
